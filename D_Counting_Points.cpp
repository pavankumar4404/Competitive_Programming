...
void solve() {
    inp2(n,m)
    vector<pi> arr(n);   
    for0(i, n) cin >> arr[i].first;
    for0(i, n) cin >> arr[i].second;
    
    map<int, int> mp;
    
    for0(i, n) {
        int x = arr[i].first, r = arr[i].second;
        for(int y = -r; y <= r; y++) {
            int xr = floor(sqrt(r * r - y * y));
            // int l = x - xr, r = x + xr;
            // for (int x_pos = l; x_pos <= r; x_pos++) {
                mp[x + y] = max(mp[x + y], xr);
            // }
        }
    }    
    ll ans = 0;
