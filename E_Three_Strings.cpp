...
ll helper(int i, int j, string &a, string &b, string &c, vector<vector<ll>> &dp){
    int k = i+j;
    if (i == n && j == m) {
        return 0;
    }
    ...
    ll ans = INT_MAX;
    if(i < n && a[i] == c[k]){
        ans = min(ans, helper(i+1, j, a, b, c, dp));
    }
    if(j < m && b[j] == c[k]){
        ans = min(ans, helper(i, j+1, a, b, c,dp));
    }
    if(i < n && a[i] != c[k]){
        ll res = helper(i + 1, j, a, b, c, dp);
        ans = min(ans, 1 + res);
    }
    if(j < m && b[j] != c[k]){
        ll res = helper(i, j + 1, a, b, c, dp);
        ans = min(ans, 1 + res);
    }
    return dp[i][j] = ans;
}
