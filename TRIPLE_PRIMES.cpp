...
bool find(ll n, const vector<int>& primes, const unordered_set<int>& st) {
    for (size_t i = 0; i < primes.size(); ++i) {
        for (size_t j = i + 1; j < primes.size(); ++j) {
            ll a = primes[i];
            ll b = primes[j];
            ll c_square = n - a * a - b * b;
            if (c_square > 0) {
                ll c = static_cast<ll>(sqrt(c_square));
                if (c * c == c_square && st.find(c) != st.end() && c != a && c != b) {
                    return true;
                }
            }
        }
