...
void solve(){
    ...
    int orig = 0;
    for(int i=1;i<n;i++){
        orig += abs(arr[i-1]-arr[i]);
    }

    int g = 0;
    for(int i=0;i<n;i++){
        int ld,rd,cd;
        if(i>0) ld = abs(arr[i]-arr[i-1]);
        else ld=0;
        if(i<n-1) rd = abs(arr[i]-arr[i+1]);
        else rd=0;
        cd=ld+rd;

        int nld, nrd, ncd;
        if(i>0) nld = abs(1-arr[i-1]);
        else nld=0;
        if(i<n-1) nrd = abs(1-arr[i+1]);
        else nrd=0;
        ncd=nld+nrd;
        g=max(g, ncd-cd);

        int nldk, nrdk, ncdk;
        if(i>0) nldk = abs(k-arr[i-1]);
        else nldk=0;
        if(i<n-1) nrdk = abs(k-arr[i+1]);
        else nrdk=0;
        ncdk=nldk+nrdk;
        g=max(g, ncdk-cd);
    }
