...
int minOccupiedDiagonals(int n, int k) {
    ...
    int full_diagonals = (k + n - 1) / n; // This ensures the correct division, ceil(k / n)
    
    return min(full_diagonals, 2 * n - 1);
}

void solve(){
    inp2(n, k)
    int ans = minOccupiedDiagonals(n,k);
    // for(int i = 0; i<n; i++){
    //     if(i == 0) k -= n;
    //     else k -= (2 * (n-i));
    //     if(k==-1 && i>0){
