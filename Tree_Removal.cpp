...
void solve() {
    ...
    while (T--) {
        ...
        auto cmp = [](const Vtx &a, const Vtx &b) {
            return (a.val == b.val) ? (a.deg > b.deg) : (a.val < b.val);
        };
        priority_queue<Vtx, vector<Vtx>, decltype(cmp)> pq(cmp);
        for (int i = 0; i < n; ++i) {
            v[i].deg = deg[i];
            if (v[i].deg % 2 == 1) {
                pq.push(v[i]);
        ...
        while (!pq.empty()) {
            ...
            if (v[cur.id].deg % 2 == 0) continue;

            maxi += cur.val;
            ans.push_back(cur.id + 1);

            for (int nbr : v[cur.id].edg) {
                v[nbr].deg--;
                if (v[nbr].deg % 2 == 1) {
                    pq.push(v[nbr]);
                }
